...
template <typename T>
void DBConnector::addToData(const char* key, T value) {
    data[key] = value;
}

template <typename T>
void DBConnector::addToData(std::string& key, T value) {
    data[key] = value;
}
...
void DBConnector::sendData(const char* subAddress, KEYSPACEPOLICIES keySpacePolicy = NODEFAULT) {
